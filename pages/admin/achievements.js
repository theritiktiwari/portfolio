...
const Achievements = (props) => {
    ...
    const handleChange = (e) => {
        ...
        } else if (e.target.name === "newPosition") {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            if (newTitle && newDescription && newImage && newPosition && newDate && newLink) {
                let Add = await addDoc(collection(db, "achievement"), {
                    title: newTitle,
                ...
                if (Add.id) {
                    tst("Achievement added successfully", "success");
    ...
    const delData = async (id) => {
        try {
            alert("Are you sure you want to delete this Achievement ?");
            await deleteDoc(doc(db, "achievement", id));
            tst("Achievement deleted successfully", "success");
            router.push("/admin/achievements");
        } catch (error) {
            tst("Something went wrong", "error");
        }
    ...
    const updateFun = (title, description, image, position, date, link) => {
        setTitle(title);
    ...
    const updateData = async (id) => {
        try {
            await updateDoc(doc(db, "achievement", id), {
                title: title,
            ...
            tst("Achievement updated successfully", "success");
    ...
    return (
        <>
            ...
            <section className='container mx-auto py-5'>
                <h2 className="text-center mb-4">Manage Achievements</h2>
                <table className="table table-responsive">
                    ...
                    <tbody>
                        {achievementData && achievementData.map((val, index) => {
                            return <>
                                <tr key={index} style={{ textAlign: "justify" }}>
                                    ...
                                    <td><input className='form-control' type="text" value={val.position} name="position" readOnly /></td>
                                    ...
                                    <td>
                                        <button onClick={() => { updateFun(val.title, val.description, val.img, val.position, val.date, val.link) }} className='mx-1 p-1 bg-primary text-light' style={{ width: "35px", height: "35px", borderRadius: "50%", outline: "none", border: "none" }}><i data-bs-toggle="modal" data-bs-target={`#ID${val.id}`} className="fas fa-pen"></i></button>
                                            ...
                                            <div className="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                                                <div className="modal-content">
                                                    ...
                                                    <div className="modal-body">
                                                        ...
                                                        <input className='form-control mb-3' type="text" onChange={handleChange} placeholder='position' value={position} name="position" />
                        ...
                        <tr>
                            ...
                            <td><input type="text" onChange={handleChange} value={newPosition} placeholder={"position"} className="form-control" name='newPosition' /></td>
