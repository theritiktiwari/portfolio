...
const Projects = (props) => {
    ...
    const delData = async (id) => {
        try {
            ...
            tst("Project deleted successfully", "success");
            router.push("/admin/projects");
        } catch (error) {
            tst("Something went wrong", "error");
        }
    ...
    const updateFun = (title, description, image, starting, ending, time, link) => {
        setTitle(title);
    ...
    const updateData = async (id) => {
        try {
            ...
            tst("Project updated successfully", "success");
    ...
    return (
        <>
            ...
            <section className='container mx-auto py-5'>
                <h2 className="text-center mb-4">Manage Project</h2>
                        ...
                        {projectData && projectData.map((val, index) => {
                            return <>
                                <tr key={index} style={{ textAlign: "justify" }}>
                                    ...
                                    <td>
                                        <button onClick={() => { updateFun(val.title, val.description, val.img, val.starting, val.ending, val.time, val.link) }} className='mx-1 p-1 bg-primary text-light' style={{ width: "35px", height: "35px", borderRadius: "50%", outline: "none", border: "none" }}><i data-bs-toggle="modal" data-bs-target={`#ID${val.id}`} className="fas fa-pen"></i></button>
